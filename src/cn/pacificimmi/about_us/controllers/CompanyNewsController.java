...
public class CompanyNewsController extends Controller{
	private static Logger log = LoggerFactory.getLogger(CompanyNewsController.class);
	Prop p = PropKit.use("constants.txt");
	public void index(){
		log.debug("forward to page jobs.jsp");
		NewsParams sp = new NewsParams(this);
		/***
		 * 获取数据列表
		 */
		sp.setPageSize(20);
		...
		this.renderJsp("/views/aboutus/company-dynamics.jsp");
	}
	...
	public void detail(){
		...
		this.setAttr("pos_nav_name", "公司动态");
		this.setAttr("pos_nav_url", "/about-us/news");
	 ...
	 * @author Gorge
	 */
	private class NewsParams extends ParamsParser{

		public NewsParams(Controller ctr) {
			super(ctr);
			...
			//查找有效数据
			this.addWhereSegmentByAnd("a.delete_flag='0' and a.status = '"+p.get("publish_status_yfb")+"' and push_location like '%00240004%' and a.section_name like '%公司动态%'");
